...
const SidebarContent = () => {
  ...
  const filteredMenuItems = menuItems.filter(item => {
    // Analyst can see all items (read-only access)
    // Officer and Admin can see all items (full access)
    return true;
  });
  ...
  return (
    <>
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden mt-4 min-h-0 sidebar-scrollbar">
        ...
        <div className="flex-shrink-0 border-t border-white/10 pt-3 mt-2">
          <SidebarLink
            ...
            className={cn(
              "rounded-lg py-2.5 text-red-300 hover:bg-red-500/20 hover:text-red-200 transition-all",
              open ? "px-3" : "px-2"
            )}
          />
        </div>
